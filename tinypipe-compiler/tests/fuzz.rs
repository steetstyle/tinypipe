...
fn rand_name(rng: &mut SimpleRng) -> String {
    let names = ["x", "y", "z", "a", "b", "c", "val", "data", "total", "count", "result", "item"];
...
fn rand_stmt(rng: &mut SimpleRng, indent: &str) -> String {
    match rng.range(10) {
        ...
        2 => format!("{}{} = call(\"math.add\", a={}, b={})", indent, rand_name(rng), rand_int(rng), rand_int(rng)),
        ...
        7 => format!("{}for {} in range({}):\n{}    pass", indent, rand_name(rng), (rng.next() % 10) + 1, indent),
...
fn rand_graph(rng: &mut SimpleRng, num_stmts: usize) -> String {
    let params: Vec<String> = (0..(rng.next() as usize % 3 + 1)).map(|_| {
        format!("{}: int", rand_name(rng))
    }).collect();
...
fn run_fuzz(seed: u64, num_graphs: usize, stmts_per_graph: usize) {
    ...
    for _ in 0..num_graphs {
        ...
        // Test sanitizer
        let sanitize_result = std::panic::catch_unwind(|| {
            sanitizer::sanitize(&code)
        });
        match sanitize_result {
            Ok(_) => {}
            Err(_) => { panic_count += 1; continue; }
        }

        // Test transform (includes sanitize internally)
        let transform_result = std::panic::catch_unwind(|| {
        ...
        match transform_result {
            Ok(Ok(plan)) => {
                ...
                match validate_result {
                    Ok(_) => {
                        // Test codegen
                        let codegen_result = std::panic::catch_unwind(|| {
                            backend::codegen::codegen(plan)
                        });
    ...
    println!("  seed {}: {} ok, {} errors, {} panics (total {})",
        seed, ok_count, err_count, panic_count, total);

    assert_eq!(panic_count, 0,
        "Fuzz seed {} produced {} panics!", seed, panic_count);
}
