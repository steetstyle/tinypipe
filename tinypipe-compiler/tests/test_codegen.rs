...
fn test_e2e_simple_graph() {
    ...
    // FB binary should also be produced and be comparable in size
...
fn test_e2e_arithmetic_graph() {
    ...
    assert!(output.binary_size_bytes > 0);
    // Verify the bincode binary round-trips
    let _roundtrip = tinypipe_ir::compiled::CompiledPlan::from_bytes(&output.binary)
        .expect("bincode binary should deserialize");
...
#[test]
fn test_e2e_binary_roundtrip() {
    let code = "def graph(x: int):\n    return x";
    let output = compile(code).expect("should compile");
    // Bincode roundtrip
    let bincode_deser = tinypipe_ir::compiled::CompiledPlan::from_bytes(&output.binary).unwrap();
    assert_eq!(output.compiled, bincode_deser);
...
fn test_e2e_call_tool_graph() {
    ...
    // Should have at least 4 nodes: Input(x), CALL, CALC(result?), ACT(return)
    assert!(output.execution_order.len() >= 4);
    // Verify compiled plan has nodes
