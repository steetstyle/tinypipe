...
fn main() {
    ...
    run_bench("mock_tools factory", || {
        let _reg = mock_tools();
    }, 1000);

    // 2. Resolve a tool
    let reg = mock_tools();
    run_bench("resolve 'math.add'", || {
